...
static inline Rect3D Rect3DMake(CGFloat oX, CGFloat oY, CGFloat oZ, CGFloat width, CGFloat height, CGFloat depth)
{
    Rect3D ret;
	Vertex3D orgin3D = Vertex3DMake(oX, oY, oZ);
    Vector3D size3D = Vector3DMake(width, height, depth);
    ret.orgin = orgin3D;
    ret.size = size3D;
	return ret;
}
...
static inline GLfloat distance(Vertex3D a, Vertex3D b){
...
static void AirRoam(int transType)
{
    float speed = 0.1;
    // 追踪观察点的方向
    static GLfloat s_eye[]={0,1.0,0};
    static GLfloat s_at[]={0.0,0.0,0.0};
    static GLfloat s_angle_h = -90.0;                    //如果初始角度设置为0则初始面向X轴正方向，设置为-90面向
    static GLfloat s_angle_v = 0;
    //Z轴负方向，符合默认情况下的设计习惯。
    float PI=3.14159f;
    // 旋转请求
    if(transType == CAM_LEFT)
    {
        s_angle_h -= 2.0;                        //每按一次左键，旋转2度。
    }
    if (transType == CAM_RIGHT)
    {
        s_angle_h += 2.0;                       //每按一次右键，旋转2度。
    } 
    float rad_h =(float)(PI * s_angle_h/180.0f);                    //计算SIN和COS函数中需要的参数。
    if(transType == CAM_FORWARD)
    {
        s_eye[2] += (float)sin(rad_h) * speed;
        s_eye[0] += (float)cos(rad_h) * speed;
    }
    if(transType == CAM_BACKWARD)
    {
        s_eye[2] -= (float)sin(rad_h) * speed;
        s_eye[0] -= (float)cos(rad_h) * speed;
    }
    ...
    {
        if(s_angle_v < 88)
            s_angle_v += 2.0;                        //每按一次左键，旋转2度。
    }
    if(transType == CAM_DOWN)
    {
        if (s_angle_v > -45) {
            s_angle_v -= 2.0;                        //每按一次左键，旋转2度。
        }
    }
    float rad_v =(float)(PI * s_angle_v/180.0f);                    //计算SIN和COS函数中需要的参数。

    // 观察点
    s_at[0] = (float)(s_eye[0] + 100*cos(rad_h));    
    s_at[2] = (float)(s_eye[2] + 100*sin(rad_h));
    s_at[1] = s_eye[1] + 100* sin(rad_v);
    glLoadIdentity();
