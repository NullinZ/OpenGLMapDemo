...
@implementation ZRMaterialController
...
{
  static ZRMaterialController *sharedMaterialController;
  @synchronized(self)
  {
    if (!sharedMaterialController)
      sharedMaterialController = [[ZRMaterialController alloc] init];
...
- (id) init
{
	self = [super init];
	if (self != nil) {
		[self loadAtlasData:@"buttonsMaterial"];
	}
	return self;
}
...
-(void)loadAtlasData:(NSString*)atlasName
{
	...
	for (NSDictionary * record in itemData) {
		ZRTexturedQuad * quad = [self texturedQuadFromAtlasRecord:record atlasSize:atlasSize materialKey:atlasName];
...
{
	ZRAnimatedQuad * animation = [[ZRAnimatedQuad alloc] init];
...
{
	ZRTexturedQuad * quad = [[ZRTexturedQuad alloc] init];
